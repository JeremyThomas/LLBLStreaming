...
// ReSharper restore PossibleNullReferenceException
    ...
    private static void LogError(IQueryable query, StringBuilder errors, Exception e)
    {
      ...
      errors.AppendLine(e.Message);
		...
		/// <summary>
		///   Checks all TypedViews can be fetched using LINQ.
		/// </summary>
		/// <param name="metaData">The meta data.</param>
		/// <param name="maxNumberOfItemsToReturn">The max number of items to return.</param>
		/// <returns></returns>
		public static StringBuilder CheckAllTypedViewClassesCanBeFetchedUsingLINQ(ILinqMetaData metaData, ushort maxNumberOfItemsToReturn)
		{
			var typedViewDatasourcPropertyInfos = from prop in metaData.GetType().GetProperties()
				let iQueryableType = MetaDataHelper.FindGenericType(typeof(DataSourceBase<>), prop.PropertyType)
				where iQueryableType != null
				let elementType = MetaDataHelper.GetTypeParameterOfGenericType(iQueryableType)
				where !typeof(IEntityCore).IsAssignableFrom(elementType)
				select prop;
			var errors = new StringBuilder();
			foreach (var typedViewDatasource in typedViewDatasourcPropertyInfos)
			{
				var query = typedViewDatasource.GetValue(metaData) as ILLBLGenProQuery;
		...
		/// <summary>
		///   Checks all TypedVisew can be fetched using LINQ.
		/// </summary>
		/// <param name="metaData">The meta data.</param>
		/// <param name="maxNumberOfItemsToReturn">The max number of items to return.</param>
		/// <returns></returns>
		public static StringBuilder CheckAllTypedListClassesCanBeFetchedUsingLINQ(ILinqMetaData metaData, ushort maxNumberOfItemsToReturn)
		{
			var typedViewDatasourcPropertyInfos = from methodInfo in metaData.GetType().GetMethods(BindingFlags.Public | BindingFlags.Instance)
				let propReturnParameter = methodInfo.ReturnParameter
				where !methodInfo.ContainsGenericParameters && !methodInfo.IsSpecialName && propReturnParameter != null && !methodInfo.GetParameters().Any()
				let iQueryableType = MetaDataHelper.FindGenericType(typeof(IQueryable<>), propReturnParameter.ParameterType)
				where iQueryableType != null
				let elementType = MetaDataHelper.GetTypeParameterOfGenericType(iQueryableType)
				where !typeof(IEntityCore).IsAssignableFrom(elementType)
				select methodInfo;
			...
			foreach (var typedViewDatasource in typedViewDatasourcPropertyInfos)
			{
				var query = typedViewDatasource.Invoke(metaData, null) as ILLBLGenProQuery;
				CheckEntityCanBeFetchedUsingLINQ(query, true, maxNumberOfItemsToReturn, errors);
			}
