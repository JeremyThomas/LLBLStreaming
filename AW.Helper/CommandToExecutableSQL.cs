...
namespace AW.Helper
{
  ...
  ///   <see
  ///     cref="http://cs.rthand.com/blogs/blog_with_righthand/pages/Implementing-more-useful-tracing-for-LLBLGenPro-2.0.aspx" />
  /// </summary>
  public static class CommandToExecutableSQL
  {
    private static readonly string[] CommonKeywords = {"FROM", "WHERE", "GROUP BY", "HAVING", "INNER JOIN", "LEFT JOIN", "RIGHT JOIN"};
    private static readonly string[] ActionKeywords = {"INSERT INTO", "DELETE"};
    private static readonly string[] SelectKeywords = {"SELECT DISTINCT", "SELECT"};
